...
function redrawPaintHistory() {
  ...
  paintHistory.forEach(stroke => {
    ...
    for (let i = 0; i < stroke.points.length; i++) {
      ...
      if (i === 0) paintContext.moveTo(point.x, point.y)
...
function destroyPaintCanvas() {
  window.removeEventListener('resize', resizePaintCanvas)
  if (paintCanvas && paintCanvas.parentNode) {
    paintCanvas.parentNode.removeChild(paintCanvas)
  }
  paintCanvas = null
  paintContext = null
  paintHistory = []
...
function startPaint(event) {
  if (!window.__dragCore?.isDraggableEnabled() || !paintCanvas) return
...
function updateCursor(event) {
  if (!window.__dragCore?.isDraggableEnabled()) {
...
function handlePaintStart(event) {
  if (!window.__dragCore?.isDraggableEnabled() || isOverDraggableElement(event)) return
  event.preventDefault()
  startPaint(event)
}

function handlePaintMove(event) {
  if (!window.__dragCore?.isDraggableEnabled()) return
...
if (typeof window !== 'undefined') {
  window.__paintSystem = paintSystem
}
